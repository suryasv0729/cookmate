...
function createIdealGraph(assetGraph, config, entries) {
  ...
  function makeManualAssetToConfigLookup() {
    ...
    for (let c of config.manualSharedBundles) {
      if (c.parent != null) {
        parentsToConfig.get(_path().default.join(config.projectRoot, c.parent)).push(c);
      }
    }
    ...
    for (let c of config.manualSharedBundles.reverse()) {
      (0, _assert().default)(c.parent == null || configToParentAsset.has(c), 'Invalid manual shared bundle. Could not find parent asset.');
      ...
      if (node.type === 'asset') {
        ...
        for (let childAsset of assets) {
          ...
          if (dependency.priority === 'lazy' || childAsset.bundleBehavior === 'isolated' // An isolated Dependency, or Bundle must contain all assets it needs to load.
            ...
            if (bundleId == null) {
              ...
              if (manualSharedObject) {
                ...
                (0, _nullthrows().default)(manualBundleToInternalizedAsset.get(bundleId)).push(childAsset);
          ...
          if (parentAsset.type !== childAsset.type || dependency.priority === 'parallel' || childAsset.bundleBehavior === 'inline') {
            ...
            if (parentAsset.type !== childAsset.type && entries.has(bundleGroupRootAsset) && canMerge(bundleGroupRootAsset, childAsset) && dependency.bundleBehavior == null) {
  ...
  for (let [nodeIdA, a] of bundleGraph.nodes.entries()) {
    ...
    for (let [nodeIdB, b] of bundleGraph.nodes.entries()) {
      if (b && a !== 'root' && b !== 'root' && a !== b && typeChangeIds.has(nodeIdB) && canMerge(a, b)) {
  ...
  for (let i = 0; i < assets.length; i++) {
    ...
    if (asset.meta.isConstantModule === true) {
      ...
      reachableRoots[i].forEach(nodeId => {
        ...
        let entryBundleId = (0, _nullthrows().default)(bundleRoots.get(entry))[0];
    ...
    reachableRoots[i].forEach(nodeId => {
      ...
      if (entries.has(a) || !a.isBundleSplittable || bundleRoots.get(a) && (getBundleFromBundleRoot(a).needsStableName || getBundleFromBundleRoot(a).bundleBehavior === 'isolated')) {
        ...
        let entryBundleId = (0, _nullthrows().default)(bundleRoots.get(a))[0];
        let entryBundle = (0, _nullthrows().default)(bundleGraph.getNode(entryBundleId));
        (0, _assert().default)(entryBundle !== 'root');
        entryBundle.assets.add(asset);
        entryBundle.size += asset.stats.size;
    ...
    if (config.disableSharedBundles === false && reachableArray.length > config.minBundles) {
      ...
      for (let root of reachableArray) {
        let bundle = (0, _nullthrows().default)(bundleGraph.getNode((0, _nullthrows().default)(bundleRoots.get(root))[0]));
        (0, _assert().default)(bundle !== 'root');
        bundle.assets.add(asset);
        bundle.size += asset.stats.size;
      }
...
async function loadBundlerConfig(config, options) {
  var _modeConfig$http, _modeConfig$minBundle, _modeConfig$minBundle2, _modeConfig$maxParall, _modeConfig$disableSh, _modeConfig$unstable_;
  ...
  return {
    ...
    manualSharedBundles: (_modeConfig$unstable_ = modeConfig.unstable_manualSharedBundles) !== null && _modeConfig$unstable_ !== void 0 ? _modeConfig$unstable_ : defaults.manualSharedBundles
  };
}
